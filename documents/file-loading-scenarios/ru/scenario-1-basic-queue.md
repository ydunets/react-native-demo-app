...
- **Хранилище очереди**: `stores/downloadQueue/downloadQueueStore.ts`
- **Отслеживание прогресса**: `stores/downloadProgress/downloadProgressStore.ts`
- **Управление файлами**: `lib/files.ts` с использованием Expo File System v17+
...
3. Файлы, отсутствующие в кеше, добавляются в очередь загрузки через `downloadQueueActions.addCommands()`

### Шаг 2: Последовательная обработка
1. Очередь загрузки начинает обрабатывать файлы один за другим
...
### Постоянство
- Состояние очереди сохраняется при перезапуске приложения с помощью хранилища MMKV
- Незавершенные загрузки возобновляются при повторном открытии приложения

### Обработка ошибок
- Сетевые ошибки приостанавливают очередь с возможностью повтора
- Ошибки аутентификации останавливают обработку до их устранения
- Отдельные сбои файлов не останавливают всю очередь
...
### Сетевые условия
- Загрузки приостанавливаются при недоступности сети
- Очередь возобновляется при восстановлении соединения
- Прогресс сохраняется во время перебоев в сети
...
- Пулинг соединений через RNFetchBlob
