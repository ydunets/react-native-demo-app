...
- Приостановка очереди, обработка приоритетной загрузки и ее отмена
...
### Ключевые компоненты
- **Управление очередью**: `stores/downloadQueue/valtioState.ts`
...
### Основные функции
- **Постоянная очередь**: Valtio + MMKV через `subscribe` для постоянства между сессиями
- **Умное кеширование**: Дедупликация на основе имени файла и проверка кеша
- **Система приоритетов**: Немедленные загрузки с поддержкой отмены
- **Локальное состояние**: Активная задача загрузки хранится в ref
